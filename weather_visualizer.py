...
matplotlib.use(
    "Agg"
)  # 画面のないサーバー環境（Colab等）でも背景で安全にグラフを描画するための設定
...
def setup_japanese_font():
    ...
    if font_name:
        ...
        print(
            "【警告】日本語フォントの自動設定に失敗しました。グラフ内の文字が『□』になる可能性があります。"
        )
...
def find_header_row(ws, search_col="A", keyword="年月日時", max_search_rows=15):
    ...
    for r in range(1, max_search_rows + 1):
        ...
        if v == keyword:
            return r
    raise ValueError(
        f"ヘッダー行（{search_col}列に「{keyword}」）が見つかりませんでした。気象庁のデータか確認してください。"
    )
...
def _draw_lane_panel(ax2, phenom_df, phenom_cols, location_mapping):
    ...
    for code in range(1, 11):
        legend_handles.append(
            Patch(facecolor=PHENOM_COLORS[code], label=f"{code}: {PHENOM_LABELS[code]}")
        )
    ax2.legend(
...
def plot_temp_humid_dew(
    main_df, phenom_df, phenom_cols, location_mapping, location_name, out_path
):
    ...
    ax1.set_title(
        f"【{location_name}】気温・露点温度（左軸℃）・相対湿度（右軸%） と 各地点の現象コードの関係",
        fontsize=13,
    )
...
def plot_wind_precip(
    ...
    pmax = (
        main_df["降水量(mm)"].dropna().max()
        if not main_df["降水量(mm)"].dropna().empty
        else 1.0
    )
    ...
    ax1.set_title(
        f"【{location_name}】風速（左軸m/s）・降水量（右軸mm） と 各地点の現象コードの関係",
        fontsize=13,
    )
    ax1.legend(
        handles=[l1, Patch(color="#2980b9", alpha=0.55, label="降水量(mm)")],
        loc="upper left",
        fontsize=9,
    )
...
def plot_combo_by_month(
    main_df, phenom_df, phenom_cols, location_mapping, location_name, out_dir
):
    for ym, msub, psub in split_by_month_two(main_df, phenom_df):
        ...
        plot_temp_humid_dew(
            ...
            loc,
            os.path.join(
                out_dir, f"{location_name}_①気温・湿度・露点×現象コード_{ym}.png"
            ),
        )
        plot_wind_precip(
            msub,
...
def _try_colab_upload():
    ...
    print(
        "アップロード画面を表示しますので、解析したい気象データのExcelファイルを選択してください…"
    )
...
def _resolve_input_file(filepath):
    ...
    if "ipykernel" in sys.modules or "IPython" in sys.modules:
        try:
            entered = input(
                "入力ファイルのパスを入力してください（Enterでキャンセル): "
            ).strip()
...
def _parse_args():
    ...
    args = [
        a for a in args if a.lower().endswith((".xlsx", ".xls")) or os.path.isdir(a)
    ]
    filepath = None
    out_dir = None
    for a in args:
...
def main():
    ...
    if filepath is None:
        ...
        print(
            "ファイル名が正しく『気象データ.xlsx』になっているか、またはフォルダーに正しく配置されているか確認してください。"
        )
        sys.exit(1)
    ...
    location_name = (
        base.split("_")[0].split(" ")[0] if ("_" in base or " " in base) else base
    )

    print(
        f"データ解析成功: {location_name} (計 {len(phenom_cols)} 地点の現象レーンを検出しました)"
    )
...
def train_location_models(main_df, phenom_df, phenom_cols, location_mapping):
    ...
    for col in phenom_cols:
        ...
        if n_classes < MIN_CLASSES_PER_LOCATION:
            print(
                f"{loc_name:<14} {len(sub):>7} {n_classes:>8} {'-':>7} {'-':>6}  現象の種類が少なくスキップ"
            )
            continue
        ...
        if y_train.nunique() < 2:
            print(
                f"{loc_name:<14} {len(sub):>7} {n_classes:>8} {'-':>7} {'-':>6}  分割後クラス不足でスキップ"
            )
            continue

        # 「より正確に」との要望に対応: 木の数を増やし、葉の最小サンプル数や
        # クラス重み調整（balanced_subsample）でRandomForestのチューニングを行う
        pipe = Pipeline(
            [
        ...
        print(
            f"{loc_name:<14} {len(sub):>7} {n_classes:>8} {acc:>7.1%} {f1:>6.3f}  学習完了"
        )
        models[col] = {
            "pipe": pipe,
            "accuracy": acc,
    ...
    if models:
        ...
        print("-" * 60)
        print(
            f"学習できた地点数: {len(models)} / {len(phenom_cols)}　平均正解率: {np.mean(accs):.1%}"
        )
    else:
...
def fetch_forecast_range(
    lat: float = FORECAST_LAT, lon: float = FORECAST_LON, forecast_days: int = None
):
...
def plot_single_location_forecast(
    ...
    hist_main = main_df[
        (main_df["datetime"] >= hist_cutoff) & (main_df["datetime"] <= last_observed)
    ].reset_index(drop=True)
    hist_phenom = phenom_df[
        (phenom_df["datetime"] >= hist_cutoff)
        & (phenom_df["datetime"] <= last_observed)
    ].reset_index(drop=True)
    ...
    ax1.axvspan(
        forecast_raw["datetime"].min(),
        forecast_raw["datetime"].max(),
        color="red",
        alpha=0.06,
        zorder=0,
    )
    ...
    ax1.axvline(boundary, color="red", linewidth=1.5, linestyle=":")
    ax1.text(
        mdates.date2num(boundary),
        ax1.get_ylim()[1],
        " ここから先は予測（赤） → ",
        fontsize=9,
        color="red",
    ...
    if slash_mask.any():
        ax2.vlines(
            x_hist[slash_mask], -0.4, 0.4, color=SLASH_COLOR, linewidth=2.2, zorder=2
        )
    for code in range(1, 11):
...
def run_fog_prediction_addon(
    ...
    print("\n" + "=" * 60)
    print(
        f"■ 今後{FORECAST_DAYS}日間の気象予報から地点ごとの現象コード予測グラフを生成"
    )
    print("=" * 60)
    try:
        ...
        print(f"\n(予報取得に失敗しました: {e})")
        print(
            "→ インターネットに出られる環境（ご自身のPCやRaspberry Piなど）で実行してください。"
        )
        return
    ...
    for col in phenom_cols:
        ...
        if gap_days > 3:
            loc_name = location_mapping.get(col, col)
            print(
                f"  【注意】{loc_name}: 最後の観測記録（{last_observed.strftime('%Y/%m/%d')}）から"
        ...
        breakdown = ", ".join(
            f"{('/' if c == 0 else c)}:{n}時間" for c, n in counts.items()
        )
